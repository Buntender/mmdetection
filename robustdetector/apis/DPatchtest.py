...
def DPatch_single_gpu_test(model,
                    data_loader,
                    show=False,
                    out_dir=None,
                    show_score_thr=0.3,
    ...
    # patch = load_patch("work_dirs_data0/ssd300_voc_CAP_PERSON_PATCH/34.npy").cuda()
    # patch = load_patch("work_dirs_data0/ssd300_voc_ROBUST_CAP_ALL_PATCH/30.npy").cuda()
    patch = load_patch("work_dirs_data0/ssd300_voc_CAP_PERSON_PATCH/38.npy").cuda()

    for i, data in enumerate(data_loader):
        data['img'][0].data[0] = add_patch(data['img'][0].data[0].cuda(), patch).cpu().unsqueeze(dim=0).detach()
