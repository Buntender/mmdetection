...
def create_patch_mask_bbox(im_data, bbox, advpatch):
    width = im_data.size(-2)
    height = im_data.size(-1)
    patch_mask = torch.zeros([3,width,height])
...
def create_img_mask(in_features, patch_mask):
    mask = torch.ones([3,in_features.size(-2), in_features.size(-1)]).to(in_features.device)
...
def add_patch(in_features, my_patch):
    global patch_mask, img_mask

    # in_features: [1,3,416,416]
    if patch_mask == None:
        patch_mask = create_patch_mask(in_features, my_patch, patch_size)
        patch_mask = Variable(patch_mask.to(in_features.device), requires_grad=False)

    if img_mask == None:
        img_mask = create_img_mask(in_features, patch_mask)
        img_mask = Variable(img_mask.to(in_features.device), requires_grad=False)

    with_patch = in_features * img_mask + my_patch * patch_mask * 255
    return with_patch
