...
def _robust_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False,
                    backpropfunc = None, dir_name = None):
    ...
    for i, data in enumerate(data_loader):
        ...
        for r in range(10):
            ...
            # loss = model(**data)
            backpropfunc(loss, model)
            perturb = perturbupdater(perturb, data['img'][0].grad.to(perturb.device), ori, data['img_metas'][0].data[0][0]['img_norm_cfg'])

        with torch.no_grad():
            datarefine = {'img': data['img'][0], 'img_metas': data['img_metas'][0]}
            result = model(return_raw = True, rescale=True, **datarefine)[1]
            filename = data['img_metas'][0][0]['ori_filename']
            filename = filename.split('/')[1].split('.')[0]
            torch.save(result[3:], dir_name + filename + '.pth')
...
robust_single_gpu_getfeature = functools.partial(_robust_single_gpu_test, backpropfunc= lambda loss, model : model.module._parse_losses(loss)[0].backward())
clsloss_single_gpu_getfeature = functools.partial(_robust_single_gpu_test, backpropfunc= lambda loss, model : loss['loss_cls'][0].backward())
bboxloss_single_gpu_getfeature = functools.partial(_robust_single_gpu_test, backpropfunc= lambda loss, model : loss['loss_bbox'][0].backward())
