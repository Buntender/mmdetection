...
class CAPRunner(EpochBasedRunner):
    ...
    def train(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(self.data_loader):
            ...
            ori = self.data_batch['img'].data[0].clone().cuda().detach()
            ...
            # for i in range(max(min((self.epoch - 12) // 2, 12), 1)):
                ...
                for k in range(self.data_batch['img'].data[0].size(0)):
                    self.data_batch['img'].data[0] = add_patch(ori, self.patch).cpu()
...
class CAPOptimizerHook(OptimizerHook):
