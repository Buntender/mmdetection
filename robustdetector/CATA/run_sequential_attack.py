...
sys.path.append('../robustdetector/utils/detectors/')
from robustdetector.utils.CATA_detectors.mmdet_model_info import model_info
from robustdetector.utils.CATA_detectors.vis_tool import voc2coco
from robustdetector.utils.CATA_detectors.utils_mmdet import get_det, show_det, get_train_model, get_train_data, get_test_data, get_loss_from_dict

FOLDER = "work_dirs/CATA_VOC_FRCNN_SSD"
for ap in range(6):
    if not os.path.isdir(f"{FOLDER}/ap{ap}"):
        os.makedirs(f"{FOLDER}/ap{ap}")
...
def main():
    ...
    # model_list = ['Faster R-CNN', 'YOLOv3', 'SSD']
    # model_list = ['My Faster R-CNN', 'MyYOLOv3']
    model_list = ['My Faster R-CNN', 'MySSD']
    # model_list = ['My Faster R-CNN', 'MySSDRobust']
    ...
    dist_mat = np.load(f'robustdetector/CATA_context/{dataset}_dist_mat_avg.npy',allow_pickle=True).item()
    size_mat = np.load(f'robustdetector/CATA_context/{dataset}_size_mat_avg.npy',allow_pickle=True).item()
    size_mat_single = np.load(f'robustdetector/CATA_context/{dataset}_size_mat_single_avg.npy',allow_pickle=True).item()
    n_labels = len(co_mat)
    # Load test image ids
    ...
    # for dirpath, dirnames, filenames in os.walk("data/VOCdevkit/VOC2007/JPEGImages"):
    #     test_image_ids = [filename[:-4] for filename in filenames]



    if dataset == 'voc':
        data_root = Path("data/VOCdevkit/VOC2007/JPEGImages/")
    else:
    ...
    for im_id in tqdm(test_image_ids[0:100]):
        try:
                ...
                for attack_step in attack_plan_all:
                    ...
                    for i in range(max_iters):
                        ...
                        for model, model_train in zip(models_wb, models_train_wb):
                            ...
                            data_train = get_train_data(model, im, pert.to(device), data, bboxes_tgt, labels)

                            # if isinstance(model_train, SingleStageDetector):
                            #     data_train = get_train_data(model, im, pert.to(device), data, bboxes_tgt, labels)
                            # else:
                    ...
                    #my_save_img
                    plt.imsave(f"{FOLDER}/{attack_step}/{im_id}.jpg", np.uint8(adv))
        except:
            pass
